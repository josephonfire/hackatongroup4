...
import { Box, Typography, Avatar, Button, Grid, Card, Divider } from '@mui/material';
...
export default function Profile({ connectedSocials = [], onChange }) {
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ width: '100%', maxWidth: 1100, mb: 2, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Logo txt_logo="#fff" img_logo="#69bec4" style={{ maxWidth: 120, maxHeight: 60 }} />
      </Box>
      <Box sx={{
        width: '100%',
        maxWidth: 600,
        ...
        <Avatar sx={{ width: 90, height: 90, fontSize: 44, bgcolor: '#5edc1f', color: '#23263a', fontWeight: 900 }}>{user.name[0]}</Avatar>
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: { xs: 'center', md: 'flex-start' }, gap: 1 }}>
          <Typography variant="h4" sx={{ fontWeight: 900, color: '#23263a', letterSpacing: 1 }}>{user.name}</Typography>
          <Typography variant="subtitle1" sx={{ color: '#23263a', opacity: 0.8, fontWeight: 500 }}>{user.email}</Typography>
        </Box>
        <Button
          sx={{
            ml: { md: 'auto' },
      ...
      <Typography variant="h6" sx={{ width: '100%', maxWidth: 1100, mb: 3, fontWeight: 800, color: '#fff', textAlign: 'center', letterSpacing: 0.5 }}>
        Social Networks
      </Typography>
      <Grid container spacing={4} sx={{ width: '100%', maxWidth: 1100, mb: 4, justifyContent: 'center' }}>
        {SOCIALS.map(social => (
          <Grid item xs={12} sm={6} md={4} key={social.key}>
            <Card sx={{
              ...
              <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1, color: '#fff' }}>{social.name}</Typography>
              <Button
      ...
      <Typography variant="body2" sx={{ color: '#69bec4', opacity: 0.7, textAlign: 'center', mb: 2, background: 'transparent', boxShadow: 'none', p: 0 }}>

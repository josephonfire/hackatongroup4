...
export default function Dashboard() {
  ...
  // Mover para o topo
  const [selectedCampaignPlatform, setSelectedCampaignPlatform] = useState('all');
  ...
  const filteredCampaigns = selectedCampaignPlatform === 'all'
    ? dashboardCampaigns
    : dashboardCampaigns.filter(camp => camp.platforms.includes(selectedCampaignPlatform));
  ...
  const getTotalRevenueForTimeframe = () => {
    ...
    return filteredSeasonData.reduce((total, day) => {
      if (day[mostProfitablePlatform] && typeof day[mostProfitablePlatform].revenue === 'number') {
  ...
  return (
    <Box
      ...
      sx={{
        minHeight: '100vh',
        height: '100vh',
        background: view === 'profile' ? '#151623' : 'linear-gradient(135deg, #23263a 0%, #33363D 60%, #69bec4 100%)',
        color: cardText,
    ...
    >
      ...
      <Box component="main" ref={dashboardRef} sx={{ flexGrow: 1, p: { xs: 2, md: 4 }, color: pageText, minHeight: '100vh', maxHeight: '100vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {view === "dashboard" && (
          <motion.div
          ...
          >
            {/* Grid principal */}
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '2fr 1fr 1fr' }, gridTemplateRows: { xs: 'auto', md: '1fr 1fr' }, gap: 3, mb: 2, width: '100%' }}>
              {/* Card maior: Receita */}
              ...
              <Box sx={{ background: '#151623', color: '#69bec4', borderRadius: 5, boxShadow: '0 4px 32px 0 #69bec422', p: 3, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', minHeight: 100 }}>
                ...
                <Box sx={{ fontWeight: 700, fontSize: 16, color: '#5edc1f', letterSpacing: 1 }}>Total Reach</Box>
              </Box>
              {/* Botão Export PDF no espaço vazio do grid */}
              <Box className="pdf-hide" sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', justifyContent: 'center', minHeight: 100 }}>
                <ExportPDF
                  ...
                  customButton={
                    <Button
                      variant="contained"
                      startIcon={<PictureAsPdfIcon sx={{ fontSize: 28, color: '#69bec4' }} />}
                      sx={{
                        background: '#151623',
                        color: '#69bec4',
                        fontWeight: 900,
                        ...
                        boxShadow: '0 6px 32px 0 #69bec488, 0 1.5px 8px 0 #69bec4aa',
                        textTransform: 'none',
                        letterSpacing: 1.2,
                        position: 'relative',
                        overflow: 'hidden',
                        ...
                        '&:hover': {
                          ...
                          color: '#fff',
                          borderColor: '#69bec4',
                          boxShadow: '0 10px 40px 0 #69bec4cc, 0 2px 12px 0 #69bec4aa',
            ...
            <Box sx={{ background: '#151623', borderRadius: 6, boxShadow: '0 4px 32px 0 #69bec422', p: { xs: 2, md: 4 }, mb: 1, color: cardText, width: '100%', maxWidth: '100%', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
              <Box sx={{ flex: 2, minWidth: 0 }}>
                ...
                <Box sx={{ height: { xs: selectedTimeframe === "30days" ? 200 : 180, md: selectedTimeframe === "30days" ? 280 : 240 }, width: '100%', maxWidth: '100%', mx: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <ModernLineChart data={chartData} whiteBg={whiteBg} height={selectedTimeframe === "30days" ? 280 : 240} />
                </Box>
              </Box>
              <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Box sx={{ background: '#151623', borderRadius: 6, boxShadow: '0 4px 32px 0 #69bec422', p: { xs: 2, md: 4 }, mb: 2, color: cardText, width: '100%', maxWidth: '100%' }}>
              ...
              <ToggleButtonGroup
                ...
                sx={{ mb: 2, '& .MuiToggleButton-root': { color: '#69bec4', borderColor: '#69bec4', fontWeight: 600, fontSize: 12, px: 2, py: 0.5, background: '#151623', '&.Mui-selected': { background: '#69bec4', color: '#23263a', borderColor: '#69bec4', '&:hover': { background: '#69bec4' } }, '&:hover': { background: '#23263a', color: '#69bec4' } } }}
              >
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
                {(filteredCampaigns || []).map((campaign, index) => {
                  ...
                  if (selectedCampaignPlatform !== 'all') {
                    ...
                    const platformKey = campaign.platforms.find(p => p.toLowerCase() === selectedCampaignPlatform.toLowerCase());
                    ...
                    <Box key={index} sx={{ 
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                        ...
                        <Box sx={{ 
                          ...
                          ROI: {selectedCampaignPlatform !== 'all' && stats && stats.cost > 0
                            ? (stats.revenue / stats.cost).toFixed(2)
                            : (campaign.revenue / campaign.cost).toFixed(2)}x
                        </Box>
                      </Box>
                      ...
                      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, fontSize: 14 }}>
                        <Box>
                          ...
                          <Typography sx={{ color: '#5edc1f', fontWeight: 700 }}>
                            {selectedCampaignPlatform !== 'all' && stats && typeof stats.revenue === 'number'
                        ...
                        <Box>
                          ...
                          <Typography sx={{ color: '#d32f2f', fontWeight: 700 }}>
                            {selectedCampaignPlatform !== 'all' && stats && typeof stats.cost === 'number'
                        ...
                        <Box>
                          ...
                          <Typography sx={{ color: '#fff', fontWeight: 600 }}>
                            {selectedCampaignPlatform !== 'all' && stats && typeof stats.impressions === 'number'
                        ...
                        <Box>
                          ...
                          <Typography sx={{ color: '#fff', fontWeight: 600 }}>
                            {selectedCampaignPlatform !== 'all' && stats && typeof stats.conversions === 'number'
            ...
            <Box className="pdf-hide" sx={{ position: 'fixed', bottom: 32, right: 32, zIndex: 1300, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
              <Button
        ...
        <Dialog open={dialogOpen} onClose={handleDialogClose} PaperProps={{ sx: { borderRadius: 4, p: 1 } }}>
          <DialogTitle sx={{ fontWeight: 700, fontSize: 20, color: '#3a5c7c', textAlign: 'center', letterSpacing: 0.5 }}>
            New Campaign
          </DialogTitle>
        ...
        <DialogActions sx={{ justifyContent: 'center', pb: 2 }}>
          <Button onClick={handleDialogClose} sx={{ fontWeight: 600, px: 3, color: '#69bec4', '&:hover': { color: '#d32f2f' } }}>Cancel</Button>
          <Button onClick={handleDialogSave} variant="contained" sx={{ fontWeight: 700, px: 3, background: 'linear-gradient(120deg, #69bec4 0%, #3a5c7c 100%)', color: '#23263a', '&:hover': { background: 'linear-gradient(120deg, #3a5c7c 0%, #69bec4 100%)', color: '#23263a' } }}>Salvar</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );

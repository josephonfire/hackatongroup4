// Endpoint para acessar os dados do x
...
router.get('/', (req, res) => {
    try {
        const xData = data.filter(item => item.platform === 'X');
        
        const result = xData.map(entry => ({
            ...entry,
        ...
        res.status(500).json({ error: 'Erro ao buscar dados do X' }); 
    }
});
